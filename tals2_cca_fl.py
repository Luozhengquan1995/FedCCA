...
def tals2_cca_fl(Cxy, Cxx,Cyy,A,B, eta_x,eta_y, Ux, Uy, innerIter, outerIter, r, evalPoints,
              U_truth, Ux_truth, Uy_truth, D_truth, fmax, factorization, BannerIter, server, dp, client_ratio):
    """
    ...
    k = Ux.shape[1]
    N = server.get_sample_num()# X.shape[1]
    ...
    if i in evalPoints:
        # Evaluate metrics
        #pdb.set_trace()
        Uxx, Uyy = GS2_gen_fl(Ux, Uy, r, server, dp, client_ratio=1.0)
    ...
    # After this line X, and Y could not use again
    # Initial computation outside the loop
    tStart = time.time()
    #XU = X.T @ Ux  # 1 pass over X
    XU = server.calculate(Ux, name='X_T', option='left', dp=dp, client_ratio=1.0)
    #YU = Y.T @ Uy  # 1 pass over Y
    YU = server.calculate(Uy, name='Y_T', option='left', dp=dp, client_ratio=1.0)
    ...
    for i in range(1, outerIter + 1):
        ...
        if not factorization:
            ...
            Ux_tilde = SVRG_Factorization_fl_x(Ux @ Gt, Uy, r, BannerIter, server, dp, client_ratio)
        Ux = GS_gen_fl(Ux_tilde,'X', r, server, dp, client_ratio=1.0)
        #XU = X.T @ Ux  # 1 pass over X
        XU = server.calculate(Ux, name='X_T', option='left', dp=dp, client_ratio=1.0)
        ...
        if not factorization:
            ...
            Uy_tilde = SVRG_Factorization_fl_y(Uy @ Gt,  Ux, r, BannerIter, server, dp, client_ratio)
        Uy = GS_gen_fl(Uy_tilde, 'Y', r, server, dp, client_ratio=1.0)
        #YU = Y.T @ Uy  # 1 pass over Y
        YU = server.calculate(Uy, name='Y_T', option='left', dp=dp, client_ratio=1.0)
    ...
    U1, U2 = GS2_gen_fl(Ux, Uy, r, server, dp, client_ratio=1.0)
    U = np.vstack((U1, U2))
    D = np.linalg.solve(U.T @ B @ U, U.T @ A @ U)
    D = np.diag(np.diag(D))
