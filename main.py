...
def main(): 
    ...
    for idx in range(3):
        ...
        # Update legend strings and plotting variables
        lgStr.append(f'FedTALSBanner_ratio{client_ratio}')
        markerStr = '-' + marker[(c - 1) % len(marker)] + color[(c - 1) % len(color)]
        c += 1
        
        # Plot results on the figures
        ax_hObj.plot(time_elapsed, np.maximum(np.finfo(float).eps, obj_err), markerStr,
                     markersize=markSize, linewidth=lineWidth,alpha=0.5, label=f'FedTALS_power_sigma{sigma}_powerorder{BannerIter}')
        ax_hAngle.plot(time_elapsed, np.maximum(np.finfo(float).eps, vec_err), markerStr,
                       markersize=markSize, linewidth=lineWidth, alpha=0.5, label=f'FedTALS_power_sigma{sigma}_powerorder{BannerIter}')
        ax_hxAngle.plot(time_elapsed, np.maximum(np.finfo(float).eps, vec_err_x), markerStr,
                        markersize=markSize, linewidth=lineWidth, alpha=0.5, label=f'FedTALS_power_sigma{sigma}_powerorder{BannerIter}')
        ...
        # Update legend strings and plotting variables
        lgStr.append(f'FedTALS_power_sigma{sigma}_powerorder{BannerIter}')
        markerStr = '-' + marker[(c - 1) % len(marker)] + color[(c - 1) % len(color)]
        c += 1
        ...
        # Save the figures
        fig_hObj.savefig(f'./output/image_sigma_order/{data_name}k{k}_obj{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hAngle.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hxAngle.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle_x{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hyAngle.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle_y{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hObj2.savefig(f'./output/image_sigma_order/{data_name}k{k}_obj_passes{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hAngle2.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle_passes{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hxAngle2.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle_x_passes{BannerIter}.png', dpi=300, bbox_inches='tight')
        fig_hyAngle2.savefig(f'./output/image_sigma_order/{data_name}k{k}_angle_y_passes{BannerIter}.png', dpi=300, bbox_inches='tight')
